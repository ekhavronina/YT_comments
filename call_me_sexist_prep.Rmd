...
length(women_vocab[!women_vocab %in% labeled_vocab])  
length(women_vocab[women_vocab %in% labeled_vocab])  
```
И так, в основном словаре есть больше половины слов, содержащихся в тренировочном (число меняется с добавлением новых комментарием).
...
men_en_df$text <- str_replace_all(men_en_df$text, "[\t\\n\r]+", " ")
men_en_df$text <- str_squish(men_en_df$text)

# короткие комментарии: 

men_en_df$text[nchar(men_en_df$text)<=3]
```
...
```{r}
men_vocab <- unique(men_tokens$word)

length(men_vocab[!men_vocab %in% labeled_vocab])  
length(men_vocab[men_vocab %in% labeled_vocab])  
```
...
# получаем список комментариев, в которых содержится хотя бы одно слово из словаря нашей модели:
men_trunc <- men_trunc$comment_id

nrow(men_en_df) - length(men_trunc)

men_count <-  men_count %>%         # оставляем комментарии длиннее 2 слов
  filter(n > 2)
men_trunc <- men_count$comment_id[men_count$comment_id %in% men_trunc]

nrow(men_en_df) - length(men_trunc) # количество комментариев, которые мы уберем
```
Фильтруем и сохраняем 

```{r}
men_final <- men_en_df %>% 
  filter(comment_id %in% men_trunc) %>% 
  mutate(sent_id = row_number())

write.csv(men_final, "men_en_df.csv", fileEncoding = "utf-8", row.names = F)
```
